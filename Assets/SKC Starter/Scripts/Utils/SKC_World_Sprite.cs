...
namespace SKC.Utils
{
    ...
    public class World_Sprite
    {
        ...
        {
            World_Sprite worldSprite = new World_Sprite(null, position, new Vector3(10, 10), SKC_Assets.i.s_White, Color.green, sortingOrderDefault);
        ...
        {
            World_Sprite worldSprite = new World_Sprite(parent, localPosition, new Vector3(10, 10), SKC_Assets.i.s_White, Color.green, sortingOrderDefault);
            worldSprite.AddButton(ClickFunc, null, null);
            return worldSprite;
        }
        public static World_Sprite CreateDebugButton(Transform parent, Vector3 localPosition, Quaternion rot,string text, System.Action ClickFunc, int fontSize = 30, float paddingX = 5, float paddingY = 5)
        {
            GameObject gameObject = new GameObject("DebugButton");
            gameObject.transform.parent = parent;
            gameObject.transform.localPosition = localPosition;
            TextMesh textMesh = SKC_Utils.CreateWorldText(text, gameObject.transform, Vector3.zero, rot,fontSize, Color.white, TextAnchor.MiddleCenter, TextAlignment.Center, 20000);
            Bounds rendererBounds = textMesh.GetComponent<MeshRenderer>().bounds;

            Color color = SKC_Utils.GetColorFromString("00BA00FF");
            ...
            World_Sprite worldSprite = new World_Sprite(gameObject.transform, Vector3.zero, rendererBounds.size + new Vector3(paddingX, paddingY), SKC_Assets.i.s_White, color, sortingOrderDefault);
        ...
        {
            return new World_Sprite(null, worldPosition, localScale, SKC_Assets.i.s_White, color, 0);
        }
        public static World_Sprite Create(Vector3 worldPosition, Vector3 localScale)
        {
            return new World_Sprite(null, worldPosition, localScale, SKC_Assets.i.s_White, Color.white, 0);
        }
        public static World_Sprite Create(Vector3 worldPosition, Vector3 localScale, int sortingOrderOffset)
        {
            return new World_Sprite(null, worldPosition, localScale, SKC_Assets.i.s_White, Color.white, sortingOrderOffset);
        }
        ...
        public World_Sprite(Transform parent, Vector3 localPosition, Vector3 localScale, Sprite sprite, Color color, int sortingOrderOffset)
        {
            ...
            gameObject = SKC_Utils.CreateWorldSprite(parent, "Sprite", sprite, localPosition, localScale, sortingOrder, color);
