...
namespace SKC.Utils
{
    public static class SKC_Utils
    {
        ...
        public static Vector3 Lerp(Vector3 a, Vector3 b, float lerpTime)
        {
            Vector3 temp = Vector3.Lerp(a, b, lerpTime);
            return temp;
        }
        ...
        public static SKC_FunctionUpdater CreateWorldTextUpdater(Func<string> GetTextFunc, Vector3 localPosition, Transform parent = null)
        {
            TextMesh textMesh = CreateWorldText(GetTextFunc(), parent, localPosition);
            return SKC_FunctionUpdater.Create(() => {
        ...
        public static TextMesh CreateWorldText(string text, Transform parent = null, Vector3 localPosition = default(Vector3), Quaternion rot = default(Quaternion),int fontSize = 40, Color? color = null, TextAnchor textAnchor = TextAnchor.UpperLeft, TextAlignment textAlignment = TextAlignment.Center, int sortingOrder = sortingOrderDefault)
        {
            if (color == null) color = Color.white;
            return CreateWorldText(parent, text, localPosition, rot,fontSize, (Color)color, textAnchor, textAlignment, sortingOrder);
        }

        // Create Text in the World
        public static TextMesh CreateWorldText(Transform parent, string text, Vector3 localPosition,Quaternion rot, int fontSize, Color color, TextAnchor textAnchor, TextAlignment textAlignment, int sortingOrder)
        {
        ...
        public static void CreateWorldTextPopup(string text, Vector3 localPosition,Quaternion rot,int size,Color col,float time)
        {
            CreateWorldTextPopup(null, text, localPosition,rot ,size,col, localPosition + new Vector3(0, 10), time);
        }

        // Create a Text Popup in the World
        public static void CreateWorldTextPopup(Transform parent, string text, Vector3 localPosition,Quaternion rot, int fontSize, Color color, Vector3 finalPopupPosition, float popupTime)
        {
            TextMesh textMesh = CreateWorldText(parent, text, localPosition,rot ,fontSize, color, TextAnchor.LowerLeft, TextAlignment.Left, sortingOrderDefault);
            Transform transform = textMesh.transform;
            Vector3 moveAmount = (finalPopupPosition - localPosition) / popupTime;
            SKC_FunctionUpdater.Create(delegate () {
        ...
        public static SKC_FunctionUpdater CreateUITextUpdater(Func<string> GetTextFunc, Vector2 anchoredPosition)
        {
        ...
        public static SKC_FunctionUpdater CreateMouseDraggingAction(Action<Vector3> onMouseDragging)
        {
            return CreateMouseDraggingAction(0, onMouseDragging);
        }

        public static SKC_FunctionUpdater CreateMouseDraggingAction(int mouseButton, Action<Vector3> onMouseDragging)
        {
            bool dragging = false;
            return SKC_FunctionUpdater.Create(() => {
                ...
                if (dragging)
                {
                    onMouseDragging(SKC_Utils.GetMouseWorldPosition());
                }
                return false;
            });
        }

        public static SKC_FunctionUpdater CreateMouseClickFromToAction(Action<Vector3, Vector3> onMouseClickFromTo, Action<Vector3, Vector3> onWaitingForToPosition)
        {
            return CreateMouseClickFromToAction(0, 1, onMouseClickFromTo, onWaitingForToPosition);
        }

        public static SKC_FunctionUpdater CreateMouseClickFromToAction(int mouseButton, int cancelMouseButton, Action<Vector3, Vector3> onMouseClickFromTo, Action<Vector3, Vector3> onWaitingForToPosition)
        {
            int state = 0;
            ...
            return SKC_FunctionUpdater.Create(() => {
                if (state == 1)
                {
                    if (onWaitingForToPosition != null) onWaitingForToPosition(from, SKC_Utils.GetMouseWorldPosition());
                }
                ...
                if (Input.GetMouseButtonDown(mouseButton) && !SKC_Utils.IsPointerOverUI())
                {
                    if (state == 0)
                    {
                        state = 1;
                        from = SKC_Utils.GetMouseWorldPosition();
                    }
                    else
                    {
                        state = 0;
                        onMouseClickFromTo(from, SKC_Utils.GetMouseWorldPosition());
                    }
                }
                return false;
            });
        ...
        public static SKC_FunctionUpdater CreateMouseClickAction(Action<Vector3> onMouseClick)
        {
            return CreateMouseClickAction(0, onMouseClick);
        }

        public static SKC_FunctionUpdater CreateMouseClickAction(int mouseButton, Action<Vector3> onMouseClick)
        {
            return SKC_FunctionUpdater.Create(() => {
                if (Input.GetMouseButtonDown(mouseButton))
                {
        ...
        public static SKC_FunctionUpdater CreateKeyCodeAction(KeyCode keyCode, Action onKeyDown)
        {
            return SKC_FunctionUpdater.Create(() => {
