...
namespace SKC.Utils
{
    ...
    public class UI_BarMultiple
    {
        ...
        {
            SKC_Utils.DrawSprite(outline.color, gameObject.transform, Vector2.zero, size + new Vector2(outline.size, outline.size), "Outline");
        }
        private RectTransform SetupBar(Color barColor)
        {
            RectTransform bar = SKC_Utils.DrawSprite(barColor, gameObject.transform, Vector2.zero, Vector2.zero, "Bar");
